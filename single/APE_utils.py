    ...
    for eval_item in eval_set:
        if prompt_instruction == '':
            if CURRENT_DATASET == "movie":
                msgs = [{"role": "user", "content": gen_init_prompt_movie(eval_item["input"])}]
            elif CURRENT_DATASET == "gsm8k":
                msgs = [{"role": "user", "content": gen_init_prompt_gsm8k(eval_item["question"])}]
            else:
                exit(-1)
        else:
            if CURRENT_DATASET == "movie":
...
def evaluation_gsm8k(CURRENT_DATASET, CURRENT_RESPONSES_FILE_PATH):
    ...
    with open(CURRENT_RESPONSES_FILE_PATH, "r") as f:
        ...
        for item in response:
            try:
                if eval(item["golden"]) == eval(item["answer"]):
                    correct_num += 1
            except:
